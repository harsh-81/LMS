 ...
 const formatDate = (timeStamp) => {
  ...
  <main className="relative flex-1 p-6 pt-28">
    ...
    <header className="flex flex-col gap-3 sm:flex-row md:items-center">
      <button className={`relative rounded sm:rounded-tr-none sm:rounded-br-none sm:rounded-tl-lg sm:rounded-bl-lg 
      text-center border-2 font-semibold py-2 w-full sm:w-72 ${filter === "returned" ? 
      ...
      text-center border-2 font-semibold py-2 w-full sm:w-72 ${filter === "nonReturned" ? 
      "bg-black text-white border-black"
      : "bg-gray-200 text-black border-gray-200 hover:bg-gray-300"}`}
      ...
      >Non-Returned Books</button>
...

        {
          booksToDisplay && booksToDisplay.length > 0 ? (
            <div className="mt-5 overflow-auto bg-white rounded-md shadow-lg">
